...
class DataTransformation:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            logging.info("Reading the train & test datasets")
            ...
            ## divide the train dataset into dependant & independant features

            input_feature_train_df=train_df.drop(columns=[target_column],axis=1)
            target_feature_train_df=train_df[target_column]

            ##divide test datset into dependant & independant features

            input_feature_test_df = test_df.drop(columns=[target_column],axis=1)
            target_feature_test_df = test_df[target_column]

            logging.info("Applying preprocessing on train & test input feature dataframe")
